...
export default async function handler(req, res) {
  ...
  const prompt = `You match everyday tasks to the AI assistant best suited for them: ChatGPT, Claude, Gemini, Perplexity, or Grok. Base this on each model's realistic relative strengths as of 2026 (Claude: careful writing, nuanced or sensitive tone, careful reasoning and coding correctness; ChatGPT: broad general knowledge, fast casual answers, image generation, wide plugin ecosystem; Gemini: deep Google Workspace integration, very long documents, multimodal and search-grounded tasks; Perplexity: research questions, fact-finding, current information with cited sources; Grok: real-time information, current events, and topics tied to X/Twitter). You also rewrite the user's task into a clearer, better-structured prompt tailored to whichever AI you recommend, the same way an experienced prompt writer would, without changing what the person is actually asking for. Even if a task seems like it could benefit from several different AI tools across multiple stages (for example, a multi-week project like buying a house or planning a launch), give exactly one clear starting recommendation for the question as asked, not a multi-step plan or a list of which tool to use for each stage. Respond with ONLY valid JSON, no markdown fences, no commentary, in exactly this shape: {"recommended":"ChatGPT|Claude|Gemini|Perplexity|Grok","reason":"one or two plain, warm sentences a non-technical person would understand","runnerUp":{"name":"ChatGPT|Claude|Gemini|Perplexity|Grok","reason":"one short sentence"} or null,"confidence":"high|medium","optimizedPrompt":"a rewritten, better-structured version of the task, ready to paste into the recommended AI"}
